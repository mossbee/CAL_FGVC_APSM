...
from angular_penalty_softmax_losses import AngularPenaltySMLoss
import math
...
# ArcFace loss functions (will be initialized in main() based on config)
arcface_loss_raw = None
arcface_loss_aux = None
arcface_loss_aug = None
...
def train(**kwargs):
    ...
    for i, (X, y) in enumerate(data_loader):
        ...
        # loss
        if config.use_arcface:
            # Use ArcFace loss instead of cross-entropy
            # Prepare feature matrices for aux and aug losses
            feature_matrix_aux = torch.cat([feature_matrix, feature_matrix_aug], dim=0)
            
            batch_loss = arcface_loss_raw(feature_matrix, y) / 3. + \
                         arcface_loss_aux(feature_matrix_aux, y_aux) * 3. / 3. + \
                         arcface_loss_aug(feature_matrix_aug, y_aug) * 2. / 3. + \
